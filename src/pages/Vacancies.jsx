...
// the brand name set as a small stack of display type, one word a line;
// Fine & Country keeps its one line and takes the extra size instead
const BRAND_LINES = {
  'Fine & Country': { lines: ['Fine & Country'], size: 'text-[1.35rem] md:text-[1.5rem]' },
};
function brandLines(name) {
  return BRAND_LINES[name] || { lines: name.split(' '), size: 'text-[1rem] md:text-[1.1rem]' };
}
    ...
    >
      ...
      >
        <div className="grid grid-cols-12 gap-x-4 md:gap-x-6 items-start">
          {/* number and the brand, stacked */}
          <div className="col-span-12 md:col-span-3">
            <span className="block font-black-display font-extrabold text-[0.8rem] tracking-[0.02em] text-[#131313]/40">
              {String(i + 1).padStart(2, '0')}
            </span>
            <span
              className={`mt-3 block font-black-display font-extrabold uppercase tracking-[-0.01em] leading-[1.05] ${brandLines(v.brand).size}`}
            >
              {brandLines(v.brand).lines.map((l) => (
          ...
          <div className="col-span-12 md:col-span-6 mt-3 md:mt-0">
            ...
            <p className="mt-3 text-[0.9rem] md:text-[0.95rem] font-light leading-[1.6] text-[#131313]/60 max-w-[34em]">
              {hook(v)}
            </p>
          </div>

          {/* what it pays, big, with the arrow beside it */}
          <div className="col-span-12 md:col-span-3 mt-5 md:mt-0 flex items-start justify-between md:justify-end gap-4">
            <div className="md:text-right">
              <span className="block font-black-display font-extrabold tracking-[-0.03em] text-[2rem] md:text-[1.9rem] lg:text-[2.3rem] leading-none whitespace-nowrap">
                {pay.to}
              </span>
              <span className="mt-1.5 block text-[0.62rem] tracking-[0.16em] uppercase text-[#131313]/55 whitespace-nowrap">
                OTE from {pay.from}
              </span>
            </div>
            <span
              className="grid place-items-center shrink-0 w-11 h-11 rounded-full border border-[#131313]/20
                transition-all duration-500 group-hover:bg-[#131313] group-hover:text-white group-hover:border-transparent
